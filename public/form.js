...
function addInputListeners(){
    ...
    if(new_club_search){
        new_club_search.addEventListener('input', function () {
            ...
            else if (new_club_search.value == ""){
                let table = document.getElementById("join-club-table");
                table.innerHTML = "";
            }
        })
    }
    ...
    if(new_event_search){
        new_event_search.addEventListener('input', function () {
            ...
            else if (new_event_search.value == ""){
                let searchTable = document.getElementById("event-search-table");
        ...
        if(user){
            ...
            clubsCollection.get().then(function(querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    ...
                    doc.ref.collection('users').get().then(function (querySnapshot) {
                        querySnapshot.forEach((function (doc) {
                            if (doc.id == user.uid){
                        ...
                        if (!already_present){
                            ...
                            if (lower_name.includes(input)) {
                                resultsList.push({name,desc})
                            }
                        ...
                        for (let i = 0 ; i < resultsList.length ; i++){
                            displayClubSearch(resultsList[i]["name"],resultsList[i]["desc"]);
                        }
                    });
                })
            })
    ...
    auth.onAuthStateChanged(function(user){
        if(user){
            ...
            clubsCollection.get().then(function(querySnapshot) {
                querySnapshot.forEach(function (doc) {
                    let member = false;
                    let club_name = doc.data().club_name;
                    let docRef = doc.ref.collection("users").doc(user.uid);
                    ...
                    doc.ref.collection("Events").get().then(function (querySnapshot) {
                        querySnapshot.forEach(function (querySnapshot) {
                            let str = querySnapshot.data().event_name;
                            console.log(str)
                            console.log(!member)
                            str = str.toLowerCase();
                            let event_name = querySnapshot.data().event_name;
                            let desc = querySnapshot.data().description;
                            let start = querySnapshot.data().start_time;
                            let date = querySnapshot.data().date;
                            if (str.includes(input) && !member) {
                                resultsList.push({club_name, event_name, desc, start, date})
                            }
                        })
                        let table = document.getElementById("event-search-table");
                        table.innerHTML = "";
                        for (let i = 0; i < resultsList.length; i++) {
                            displayEvent(resultsList[i]["club_name"], resultsList[i]["event_name"], resultsList[i]["desc"], resultsList[i]["start"], resultsList[i]["date"]);
                        }
...
function displayEvent(club_name,name, desc, start, date){
    let event_table = document.getElementById("event-search-table");
...
function displayClubSearch(name, descrip)
{
    auth.onAuthStateChanged(function(user){
        if(user){
            ...
            joinButton.setAttribute("style","cursor:pointer;")
            ...
            if (my_clubs) {
                clubsCollection.get().then(function (querySnapshot) {
                    querySnapshot.forEach(function (doc) {
                        let isAdmin = false;
                        let record_exists = false;
                        doc.ref.collection("users").get().then(function (data) {
